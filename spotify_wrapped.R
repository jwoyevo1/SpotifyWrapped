...
hoursArtist <- mySpotify %>% 
  ...
  gghighlight(hours>3)
 # gghighlight(artistName == "JPEGMAFIA")#  || artistName == "MF DOOM") #|| artistName == "MF DOOM"|| artistName == "Bladee") 
hoursArtist
...
hoursDay %>% 
  ...
  geom_col(fill = "#1db940") + dark_theme_linedraw() +
...
songsByDay <- mySpotify %>% 
  ...
  scale_fill_gradient(high = "#1db940", low = "#d30000") + 
  labs(x= "Date", y= "Number of Songs Played") + 
  ggtitle("Plays per Day", "May 2022 to May 2023")
songsByDay
...
minutesTMostListened <- mySpotify %>% 
  ...
  geom_col(aes(fill = minutesListened)) + dark_theme_linedraw() +
  scale_fill_gradient(low = "#d30000", high = "#1db940") + 
